...
def crop_video(input_file, x1, y1, x2, y2):
  ...
  fourcc = cv2.VideoWriter_fourcc(*'XVID')  # Or other suitable codec
  out = cv2.VideoWriter(output_file, 0x00000021, fps, (x2 - x1, y2 - y1))
...
def get_contour_remove_black_borders(vid_link):
  ...
  fourcc = cv2.VideoWriter_fourcc(*'XVID')  # Specify the codec (adjust as needed)
  out = cv2.VideoWriter(output_file, 0x00000021, cap.get(cv2.CAP_PROP_FPS), (w, h))
